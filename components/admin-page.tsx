...
export function AdminPage() {
  ...
  useEffect(() => {
    if (user) void checkAdminAccess();
  }, [user]);
  useEffect(() => subscribeSchoolConfig("ses", setConfig), []);
  useEffect(() => {
    if (!config) return;
    const validYear =
      config.years.find((item) => item.id === yearId) || config.years[0];
    if (!validYear) {
      setYearId("");
      setClassId("");
      setHouseId("");
    ...
    const validClass =
      validYear.classes.find((item) => item.id === classId) ||
      validYear.classes[0];
    const validHouse =
      validClass?.houses.find((item) => item.id === houseId) ||
      validClass?.houses[0];
    ...
    return (
      <section className="page">
        <div className="admin-lock">
          ...
          <p>
            MPlace Study is checking for a Firestore document at{" "}
            <code>admins/{user?.uid}</code>.
          </p>
          {user?.uid ? (
            <p>
              <strong>Your current Firebase UID:</strong>
              <br />
              <code>{user.uid}</code>
            </p>
          ) : null}
          {accessError ? (
            <p>
              <strong>Firestore error:</strong> <code>{accessError}</code>
            </p>
          ) : (
            <p>No admin document was found for this signed-in account.</p>
          )}
          <button
  ...
  async function addYear() {
    ...
    await persistConfig({
      ...config,
      years: [...config.years, { id, label, classes: [] }],
    });
    setYearId(id);
    setClassId("");
    setHouseId("");
    ...
    const years = config.years.map((y) =>
      y.id === year.id
        ? {
            ...y,
            classes: [
              ...y.classes.filter((c) => c.id !== nextClass.id),
              nextClass,
            ],
          }
        : y,
  ...
  async function deleteClass() {
    ...
    if (!window.confirm(`Delete ${schoolClass.label}?`)) return;
    const remaining = year.classes.filter((c) => c.id !== schoolClass.id);
    const years = config.years.map((y) =>
      y.id === year.id ? { ...y, classes: remaining } : y,
    );
    await persistConfig({ ...config, years });
            ...
            classes: y.classes.map((c) =>
              c.id === schoolClass.id
                ? {
                    ...c,
                    houses: [
                      ...c.houses.filter((h) => h.id !== nextHouse.id),
                      nextHouse,
                    ],
                  }
                : c,
  ...
  async function addPrefix() {
    ...
    await persistConfig({
      ...config,
      roomPrefixes: [
        ...config.roomPrefixes.filter((item) => item.prefix !== prefix),
        { prefix, building },
      ],
    });
  }
  if (!config)
    return (
      <section className="page">
        <div className="admin-lock">
          <h1>Set up MPlace Study school data</h1>
          <p>No SES school configuration exists yet.</p>
          <button className="primary-button" onClick={initialise}>
            <Plus size={17} /> Initialise SES
          </button>
        </div>
      </section>
    );
    ...
    <section className="page">
      <div className="page-head">
        <div>
          ...
          <h1>MPlace Study Admin</h1>
          <p>
            Manage announcements, school structure, Class Locator and the Master
            Calendar.
          </p>
        </div>
      </div>
      ...
      <AdminMasterCalendar />
      <AdminAnnouncements adminUid={user?.uid || ""} />
      <section className="admin-section">
        <div className="section-row">
          <div>
            <h2 className="section-title">School structure</h2>
            <p className="section-help">
              Manage years, classes and houses used elsewhere in MPlace Study.
            </p>
          </div>
        </div>
        {config.years.length === 0 ? (
          <div className="school-empty">
            No valid years remain. Use Add year to rebuild the structure.
          </div>
        ) : null}
        ...
        <div className="form-actions">
          <button className="text-button" onClick={addYear}>
            <Plus size={15} /> Add year
          </button>
          <button className="text-button" onClick={addClass} disabled={!year}>
      ...
      <section className="admin-section">
        <div className="section-row">
          <div>
            ...
            <p className="section-help">
              Room prefixes are used by personal timetable entries and Class
              Locator.
            </p>
          </div>
          <button className="text-button" onClick={addPrefix}>
            <Plus size={15} /> Add prefix
          </button>
        </div>
        <div className="locator-prefix-grid">
          {config.roomPrefixes.map((item) => (
            <article key={item.prefix}>
              ...
              <button
                ...
                onClick={() =>
                  void persistConfig({
                    ...config,
                    roomPrefixes: config.roomPrefixes.filter(
                      (p) => p.prefix !== item.prefix,
                    ),
                  })
                }
              >
