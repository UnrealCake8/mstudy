...
function generateQuestions(text: string): Question[] {
  ...
  for (const sentence of sentences) {
    ...
    questions.push({ id: `${questions.length}-${answer}`, prompt: sentence.replace(new RegExp(`\\b${escaped}\\b`, "i"), "_____"), choices: shuffle([answer, ...distractors]), answer, explanation: sentence });
...
export function PlayPage() {
  ...
  async function startGame() {
    ...
    if (aiEnabled) {
      try {
        ...
        generated = data.questions.map((q, i) => ({ ...q, id: `ai-${i}` }));
      } catch (error) {
    ...
    setQuestions(generated); setIndex(0); setBossHp(1000); setHearts(3); setScore(0); setStreak(0); setCorrectCount(0); setAnsweredCount(0); setPicked(null); savedRun.current = false; setStage("battle");
  }

  function answer(choice: string) {
    if (picked || !current) return; setPicked(choice); setAnsweredCount(v => v + 1);
    const correct = choice.toLowerCase() === current.answer.toLowerCase();
    if (correct) { const nextStreak = streak + 1, damage = Math.min(220, 110 + nextStreak * 15); setCorrectCount(v => v + 1); setStreak(nextStreak); setBossHp(h => Math.max(0, h - damage)); setScore(s => s + 100 + nextStreak * 20); }
    else { setStreak(0); setHearts(h => Math.max(0, h - 1)); }
  }
  function next() { if (!picked) return; if (bossHp <= 0 || hearts <= 0 || index >= questions.length - 1) { setStage("results"); return; } setIndex(i => i + 1); setPicked(null); }
  ...
  const correctPick = picked && current && picked.toLowerCase() === current.answer.toLowerCase();
  return <section className="battle-shell"><header className="battle-topbar"><div><button className="icon-button" onClick={() => setStage("setup")} aria-label="Exit battle"><X size={18}/></button><div><strong>{title}</strong><small>{subject}</small></div></div><div className="battle-stats"><span><Heart size={15}/> {hearts}</span><span><Sparkles size={15}/> x{streak}</span><span>{score} pts</span></div></header><div className="boss-zone"><div className={`boss-avatar ${correctPick ? "boss-hit" : ""}`}>{bossImage ? <img src={bossImage} alt={bossName || "Custom boss"}/> : <Swords size={50}/>}</div><strong>{bossName || "REVISION BOSS"}</strong><div className="boss-health"><span style={{ width: `${bossHp / 10}%` }}/></div><small>{bossHp} HP</small></div>{current ? <article className="question-card"><div className="question-meta"><span>Question {index + 1} / {questions.length}</span><span>{subject}</span></div><h2>{current.prompt}</h2><div className="choice-grid">{current.choices.map(choice => { const chosen = picked === choice; const isAnswer = choice.toLowerCase() === current.answer.toLowerCase(); return <button key={choice} disabled={Boolean(picked)} onClick={() => answer(choice)} className={`choice ${picked ? (isAnswer ? "correct" : chosen ? "wrong" : "") : ""}`}>{choice}</button>; })}</div>{picked ? <div className={`answer-feedback ${correctPick ? "correct" : "wrong"}`}><strong>{correctPick ? "Direct hit!" : "Not quite."}</strong><span>{current.explanation}</span><button className="primary-button" onClick={next}>{bossHp <= 0 || hearts <= 0 || index >= questions.length - 1 ? "See results" : "Next attack"}</button></div> : null}</article> : null}</section>;
}
